...
def homePage():
    return "<h2> simple audio fingerprinting api testing page </h2>"
@app.route("/getAudio",methods=["POST"])
def getAudioFingerprinting():
    try:
        global db
        getFile=request.files['audio']
        uploads_dir=os.path.join(app.root_path,'uploads',secure_filename("sample.mp3"))
        getFile.save(uploads_dir)
        song=os.path.join(app.root_path,'uploads','sample.mp3')
